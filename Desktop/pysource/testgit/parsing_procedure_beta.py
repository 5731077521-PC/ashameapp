...
def test_predict_fn():
  df_test = pd.read_csv("~/Desktop/data/"+str(sys.argv[1]),names=COLUMNS, skiprows=1)
  #df_test[LABEL_COLUMN] = (df_test["Tag"].apply(lambda x: "Botnet" in x)).astype(int)
  #df_test["SrcAddr"] = ((df_test["SrcAddr"].astype('str')).apply(ip2long)).astype(int)
  #df_test["StartTime"] = ((df_test["StartTime"].astype('str')).apply(stamp2epoch)).astype(int)
...
def main():
    ...
    with tf.Session() as sess:
      ...
      #writes = csv.writer(open("/home/netklause/Desktop/data/sandbox/prob_dump/"+str(sys.argv[2]), 'w', newline=''), delimiter=',', quoting=csv.QUOTE_ALL)
      ...
      for i in range(len(result.index)) :
        if(result['botnet'].loc[i]>1e-20):
          SrcAddr_list.append(result['SrcAddr'].loc[i])
          val_list.append(result['botnet'].loc[i])
          line_list.append(i+2)
          idx = idx+1
          """notify['SrcAddr'].loc[idx] = result['SrcAddr'].loc[i]
          notify['val'].loc[idx] = result['botnet'].loc[i]
          notify['line'].loc[idx] = i+2
          idx = idx+1"""

      notify['SrcAddr'] = SrcAddr_list
      notify['val'] = val_list
      notify['line'] = line_list


      notify.to_csv('/home/netklause/Desktop/data/sandbox/prob_dump/notify.csv')

      #for(i in range(len(result.index))):

      """derived_pred_df['SrcAddr'] = df_SrcAddr
      derived_pred_df['normal'] = pred_df['normal'] 
      derived_pred_df['botnet'] = pred_df['botnet']"""
