...
COLUMNS = ["NewAddr","SrcRate","SrcAddr","sHops","dHops","sTtl","dTtl","Proto","State","Tag"]
LABEL_COLUMN = "label"
CATEGORICAL_COLUMNS = ["Proto","State","Tag"]
CONTINUOUS_COLUMNS = ["SrcRate","NewAddr","SrcAddr","sHops","dHops","sTtl","dTtl"]
IP_COLUMNS = ["NewAddr"]
...
def train_input_fn():
  ...
  df_train["NewAddr"] = ((df_train["SrcAddr"].astype('str')).apply(ip2long)).astype(int)
  """
  df2 = pd.read_csv(train_file,names="StartTime", skiprows=1)
  df2 = df2.applymap(str)
  df2 = (df2.apply(stamp2epoch)).astype(int)
  
  df_train = df_train.assign(StartTime=pd.Series(df2))
  """
  return input_fn(df_train)

def eval_input_fn():
  test_file = '~/Desktop/pysource/eval4.csv'
  ...
  df_test["NewAddr"] = ((df_test["SrcAddr"].astype('str')).apply(ip2long)).astype(int)
  """
  df2 = pd.read_csv(train_file,names="StartTime", skiprows=1)
  df2 = df2.applymap(str)
  df2 = (df2.apply(stamp2epoch)).astype(int)
  
  df_test = df_test.assign(StartTime=pd.Series(df2))
  """
...
def main():
    ...
    NewAddr = tf.contrib.layers.real_valued_column("NewAddr")
    ...
    m = tf.contrib.learn.LinearClassifier(feature_columns=[SrcRate,NewAddr,sHops,dHops,sTtl,dTtl],
    model_dir=model_dir)
